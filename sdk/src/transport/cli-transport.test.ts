...
describe("CliTransport", () => {
  ...
  describe("constructor", () => {
    it("should use default binaryPath 'agnetd'", () => {
      ...
      mockExecFile.mockResolvedValue({ stdout: '{"data":{"ok":true},"success":true}', stderr: "" });
      transport.execute(["test"]);
      expect(mockExecFile).toHaveBeenCalledWith("agnetd", expect.any(Array), expect.any(Object));
    });

    it("should use custom binaryPath", () => {
      const transport = new CliTransport({ binaryPath: "/usr/local/bin/agnetd" });
      mockExecFile.mockResolvedValue({ stdout: '{"data":{"ok":true},"success":true}', stderr: "" });
      transport.execute(["test"]);
      expect(mockExecFile).toHaveBeenCalledWith("/usr/local/bin/agnetd", expect.any(Array), expect.any(Object));
    });
    ...
    it("should pass global --identity flag", () => {
      ...
      mockExecFile.mockResolvedValue({ stdout: '{"data":{},"success":true}', stderr: "" });
  ...
  describe("execute", () => {
    it("should always pass --output json-compact", () => {
      const transport = new CliTransport();
      mockExecFile.mockResolvedValue({ stdout: '{"data":{"x":1},"success":true}', stderr: "" });
    ...
    it("should parse a single success envelope", async () => {
      ...
      mockExecFile.mockResolvedValue({
        stdout: '{"data":{"DID":"did:neunode:abc","Name":"test"},"success":true}',
        stderr: "",
      });
      const result = await transport.execute<{ DID: string; Name: string }>(["identity", "show"]);
    ...
    it("should throw CliTransportError on error envelope in stdout", async () => {
      ...
      await expect(transport.execute(["identity", "show"])).rejects.toThrow(CliTransportError);
      await expect(transport.execute(["identity", "show"])).rejects.toThrow("agent not found");
    });
    ...
    it("should throw CliTransportError on error envelope in stderr", async () => {
      ...
      await expect(transport.execute(["config", "get", "x"])).rejects.toThrow("config not found");
    });

    it("should throw CliTransportError on empty stdout", async () => {
      const transport = new CliTransport();
      mockExecFile.mockResolvedValue({ stdout: "   ", stderr: "" });
      await expect(transport.execute(["test"])).rejects.toThrow("Empty response from agnetd");
    });
    ...
    it("should handle child process error with stderr containing error envelope", async () => {
      const transport = new CliTransport();
      const err = new Error("exit code 1") as NodeJS.ErrnoException & { stderr: string };
      err.code = "ERR_CHILD_PROCESS";
      err.stderr = '{"error":"invalid argument","success":false}';
      mockExecFile.mockRejectedValue(err);
      await expect(transport.execute(["bad"])).rejects.toThrow("invalid argument");
    });
    ...
    it("should handle child process error with non-JSON stderr", async () => {
      ...
      mockExecFile.mockRejectedValue(err);
      await expect(transport.execute(["bad"])).rejects.toThrow(CliTransportError);
    });
    ...
    it("should pass command args after global flags", () => {
      ...
      expect(args).toEqual([
        "--output", "json-compact",
        "--identity", "did:neunode:abc",
        "identity", "show", "--did", "did:neunode:xyz",
      ]);
    });
  });
  ...
  describe("executeMulti", () => {
    it("should return array of data payloads from multiple envelopes", async () => {
      const transport = new CliTransport();
      mockExecFile.mockResolvedValue({
        stdout: '{"data":{"token":"nCompute","balance":"1000"},"success":true}\n{"data":{"token":"nTrain","balance":"500"},"success":true}',
        stderr: "",
      });
      const results = await transport.executeMulti<Record<string, string>>(["token", "balance", "--token", "nCompute"]);
    ...
    it("should throw on error envelope in stderr for multi-output", async () => {
      ...
      await expect(transport.executeMulti(["test"])).rejects.toThrow("stderr error");
    });
    ...
    it("should skip lines that are not valid JSON envelopes", async () => {
      ...
      const results = await transport.executeMulti<Record<string, boolean>>(["test"]);
    ...
    it("should handle child process error in multi with stderr error envelope", async () => {
      ...
      mockExecFile.mockRejectedValue(err);
      await expect(transport.executeMulti(["bad"])).rejects.toThrow("multi-fail");
    });

    it("should handle generic error in multi without stderr", async () => {
      const transport = new CliTransport();
      mockExecFile.mockRejectedValue("multi-error");
      await expect(transport.executeMulti(["bad"])).rejects.toThrow(CliTransportError);
    });
  });
  ...
  describe("executeRaw", () => {
    ...
    it("should throw CliTransportError on child process error", async () => {
      ...
      mockExecFile.mockRejectedValue(err);
      await expect(transport.executeRaw(["bad"])).rejects.toThrow(CliTransportError);
    });
