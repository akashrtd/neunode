...
describe("createIdentityResource", () => {
  ...
  it("should throw if cli transport is missing", () => {
    ...
    expect(() => createIdentityResource(noCliClient)).toThrow("CLI transport required");
  });
  ...
  describe("create", () => {
    ...
    it("should pass --method when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "identity", "create", "--name", "test", "--method", "key",
      ]);
    });
    ...
    it("should pass --output-dir when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "identity", "create", "--name", "test", "--output-dir", "/tmp/keys",
      ]);
    });
  ...
  describe("show", () => {
    ...
    it("should pass --did when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith(["identity", "show", "--did", "did:neunode:abc"]);
    });
  });
  ...
  describe("export", () => {
    it("should call execute with --file only", async () => {
      execute.mockResolvedValue({ did: "did:neunode:abc", exported_at: "2026-01-01" });
      const resource = createIdentityResource(mockClient);
      await resource.export({ file: "/tmp/export.json" });
      expect(execute).toHaveBeenCalledWith(["identity", "export", "--file", "/tmp/export.json"]);
    });

    it("should pass --did when provided", async () => {
      execute.mockResolvedValue({ did: "did:neunode:abc" });
      const resource = createIdentityResource(mockClient);
      await resource.export({ file: "/tmp/export.json", did: "did:neunode:abc" });
      expect(execute).toHaveBeenCalledWith([
        "identity", "export", "--file", "/tmp/export.json", "--did", "did:neunode:abc",
      ]);
    });
  });
});
