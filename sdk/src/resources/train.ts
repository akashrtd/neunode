...
export interface TrainResource {
  ...
  coordinatorStatus(params: CoordinatorStatusParams): Promise<CoordinatorStatusResult>;
}
...
export function createTrainResource(client: NeunodeClient): TrainResource {
  ...
  return {
    ...
    async registerWorker(params: WorkerRegisterParams): Promise<WorkerRegisterResult> {
      const args = [
        "train", "worker-register",
        "--gpu-count", String(params.gpuCount),
        "--gpu-memory", String(params.gpuMemoryGb),
        "--max-params", String(params.maxModelParams),
      ];
    ...
    async listWorkers(params?: WorkerListParams): Promise<WorkerListResult> {
      ...
      if (params?.minMemory) args.push("--min-memory", String(params.minMemory));
      return cli.execute<WorkerListResult>(args);
    },

    async coordinatorStatus(params: CoordinatorStatusParams): Promise<CoordinatorStatusResult> {
      return cli.execute<CoordinatorStatusResult>(["train", "coordinator-status", "--job-id", params.jobId]);
    },
  };
}
