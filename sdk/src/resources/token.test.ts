...
describe("createTokenResource", () => {
  ...
  it("should throw if cli transport is missing", () => {
    expect(() => createTokenResource({ ...mockClient, cli: undefined })).toThrow("CLI transport required");
  });
  ...
  describe("balance", () => {
    ...
    it("should pass --token when provided", async () => {
      execute.mockResolvedValue({ token: "nCompute", balance: "1000", staked: "100" });
      const resource = createTokenResource(mockClient);
      await resource.balance("nCompute");
      expect(execute).toHaveBeenCalledWith(["token", "balance", "--token", "nCompute"]);
    });
  });

  describe("transfer", () => {
    it("should call execute with token transfer --to --amount --token", async () => {
      execute.mockResolvedValue({ from: "did:neunode:abc", to: "did:neunode:def", amount: 100, token: "nCompute", state: "sent" });
      const resource = createTokenResource(mockClient);
      await resource.transfer({ to: "did:neunode:def", amount: 100, token: "nCompute" });
      expect(execute).toHaveBeenCalledWith([
        "token", "transfer", "--to", "did:neunode:def",
        "--amount", "100", "--token", "nCompute",
      ]);
    });
  });
  ...
  describe("stake", () => {
    it("should call execute with token stake --amount --token", async () => {
      execute.mockResolvedValue({ amount: 500, token: "nCompute", state: "staked", unbonding_period_secs: 86400 });
      const resource = createTokenResource(mockClient);
      await resource.stake({ amount: 500, token: "nCompute" });
      expect(execute).toHaveBeenCalledWith([
        "token", "stake", "--amount", "500", "--token", "nCompute",
      ]);
    });
  });
  ...
  describe("unstake", () => {
    it("should call execute with token unstake --amount", async () => {
      execute.mockResolvedValue({ amount: 200, token: "nCompute", unbond_at: 1234567890, state: "unbonding" });
      const resource = createTokenResource(mockClient);
      await resource.unstake(200);
      expect(execute).toHaveBeenCalledWith(["token", "unstake", "--amount", "200"]);
    });
  });
