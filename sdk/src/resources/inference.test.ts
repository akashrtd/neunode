...
describe("createInferenceResource", () => {
  ...
  it("should throw if cli transport is missing", () => {
    expect(() => createInferenceResource({ ...mockClient, cli: undefined })).toThrow("CLI transport required");
  });
  ...
  describe("request", () => {
    it("should call execute with inference request (required params)", async () => {
      ...
      await resource.request({ model: "llama-3b", prompt: "hello", maxTokens: 512 });
      expect(execute).toHaveBeenCalledWith([
        "inference", "request", "--model", "llama-3b", "--prompt", "hello", "--max-tokens", "512",
      ]);
    });
    ...
    it("should pass --temperature when provided", async () => {
      execute.mockResolvedValue({ model: "llama-3b", status: "routed" });
      const resource = createInferenceResource(mockClient);
      await resource.request({ model: "llama-3b", prompt: "hello", maxTokens: 512, temperature: 0.7 });
      expect(execute).toHaveBeenCalledWith([
        "inference", "request", "--model", "llama-3b", "--prompt", "hello",
    ...
    it("should NOT pass --temperature when undefined", async () => {
      ...
      const resource = createInferenceResource(mockClient);
      await resource.request({ model: "llama-3b", prompt: "hi", maxTokens: 100 });
    ...
    it("should pass --temperature when 0 (falsy but defined)", async () => {
      execute.mockResolvedValue({ model: "llama-3b" });
      const resource = createInferenceResource(mockClient);
      await resource.request({ model: "llama-3b", prompt: "hi", maxTokens: 100, temperature: 0 });
      const callArgs = execute.mock.calls[0]?.[0] as string[] | undefined;
      expect(callArgs).toBeDefined();
  ...
  describe("listModels", () => {
    ...
    it("should pass --provider when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith(["inference", "list-models", "--provider", "provider-abc"]);
    });
  });
  ...
  describe("providers", () => {
    ...
    it("should pass --model when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith(["inference", "providers", "--model", "llama-3b"]);
    });
  });
  ...
  describe("route", () => {
    it("should default strategy to 'cheapest'", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "inference", "route", "--model", "llama-3b", "--strategy", "cheapest",
      ]);
    });
    ...
    it("should use provided strategy", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "inference", "route", "--model", "llama-3b", "--strategy", "Fastest",
      ]);
    });
  });
  ...
  describe("pricing", () => {
    it("should call execute with inference pricing --model --input-tokens --output-tokens", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "inference", "pricing", "--model", "llama-3b",
