...
describe("createModelResource", () => {
  ...
  it("should throw if cli transport is missing", () => {
    expect(() => createModelResource({ ...mockClient, cli: undefined })).toThrow("CLI transport required");
  });
  ...
  describe("list", () => {
    ...
    it("should pass --provider when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith(["model", "list", "--provider", "provider-abc"]);
    });
  });

  describe("show", () => {
    it("should call execute with model show --model-id", async () => {
      execute.mockResolvedValue({ "Model ID": "llama-3b", "Base Model": "llama" });
      const resource = createModelResource(mockClient);
      await resource.show("llama-3b");
      expect(execute).toHaveBeenCalledWith(["model", "show", "--model-id", "llama-3b"]);
    });
  });
  ...
  describe("push", () => {
    it("should call execute with model push --path --name", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "model", "push", "--path", "/tmp/model.safetensors", "--name", "my-model",
      ]);
    });
  });
  ...
  describe("rm", () => {
    it("should call execute with model rm --model-id", async () => {
      execute.mockResolvedValue({ action: "rm", model_id: "my-model", status: "ok" });
      const resource = createModelResource(mockClient);
      await resource.rm("my-model");
      expect(execute).toHaveBeenCalledWith(["model", "rm", "--model-id", "my-model"]);
    });
  });
});
