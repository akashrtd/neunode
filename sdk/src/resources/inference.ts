...
export interface InferenceResource {
  ...
  pricing(model: string, inputTokens: number, outputTokens: number): Promise<InferencePricingResult>;
}

export function createInferenceResource(client: NeunodeClient): InferenceResource {
  const cli = client.cli;
  if (!cli) throw new Error("CLI transport required for inference operations");

  return {
    async request(params: InferenceRequestParams): Promise<InferenceRequestResult> {
      const args = [
        "inference", "request",
        "--model", params.model,
    ...
    async route(model: string, strategy?: string): Promise<InferenceRouteResult> {
      const args = ["inference", "route", "--model", model, "--strategy", strategy ?? "cheapest"];
      return cli.execute<InferenceRouteResult>(args);
    },

    async pricing(model: string, inputTokens: number, outputTokens: number): Promise<InferencePricingResult> {
