...
describe("createFeedResource", () => {
  ...
  it("should throw if cli transport is missing", () => {
    expect(() => createFeedResource({ ...mockClient, cli: undefined })).toThrow("CLI transport required");
  });
  ...
  describe("post", () => {
    it("should call execute with feed post --kind and --content", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "feed", "post", "--kind", "1000", "--content", '{"title":"hello"}',
      ]);
    });
    ...
    it("should pass --tags for each tag", async () => {
      execute.mockResolvedValue({ "Event ID": "evt_123" });
      const resource = createFeedResource(mockClient);
      await resource.post({ kind: 1000, content: "test", tags: ["ai", "training"] });
      expect(execute).toHaveBeenCalledWith([
        "feed", "post", "--kind", "1000", "--content", "test",
        "--tags", "ai", "--tags", "training",
      ]);
    });
  ...
  describe("list", () => {
    ...
    it("should pass --kind, --author, --limit when provided", async () => {
      ...
      expect(execute).toHaveBeenCalledWith([
        "feed", "list", "--kind", "1000", "--author", "did:neunode:abc", "--limit", "10",
      ]);
    });
  ...
  describe("show", () => {
    it("should call execute with feed show --event-id", async () => {
      ...
      expect(execute).toHaveBeenCalledWith(["feed", "show", "--event-id", "evt_abc123"]);
    });
  });

  describe("subscribe", () => {
    it("should call execute with feed subscribe (no kind)", async () => {
      execute.mockResolvedValue({ topic: "feed", status: "active", streaming: true });
      const resource = createFeedResource(mockClient);
    ...
    it("should pass --kind when provided", async () => {
      execute.mockResolvedValue({ topic: "bounty", status: "active", streaming: true });
      const resource = createFeedResource(mockClient);
      await resource.subscribe(1000);
      expect(execute).toHaveBeenCalledWith(["feed", "subscribe", "--kind", "1000"]);
    });
  });
});
