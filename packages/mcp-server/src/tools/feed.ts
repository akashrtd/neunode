...
export function registerFeedTools(
  ...
  server.tool(
    "neunode_subscribe_feed",
    "Read recent feed events as a snapshot (for monitoring the feed)",
    {
      kind: z
        .number()
        .int()
  ...
  server.tool(
    "neunode_search_feeds",
    "Search feed events by kind, author, or other criteria",
    {
      kind: z
        .number()
        .int()
      ...
      author: z
        .string()
        .optional()
        .describe("Filter by author DID substring"),
      limit: z
        .number()
        .int()
        .min(1)
    ...
    async ({ kind, author, limit }) => {
      const result = await client.searchFeeds({ kind, author, limit });
      return {
        content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }],
      };
    },
