...
pub fn build_api_router() -> Router<Arc<ApiState>> {
    Router::new()
        ...
        .route(
            "/api/v1/identity/register-onchain",
            post(super::identity_api::register_onchain),
        )
        // Feed
        ...
        .route(
            ...
            post(super::inference_api::request_inference),
        )
        .route(
            "/api/v1/inference/models",
            get(super::inference_api::list_models),
        )
        .route(
            "/api/v1/inference/providers",
            get(super::inference_api::list_providers),
        )
        ...
        .route(
            ...
            get(super::knowledge_api::query_knowledge),
        )
        .route(
            "/api/v1/knowledge/register-agent",
            post(super::knowledge_api::register_agent),
        )
        .route(
            "/api/v1/knowledge/register-model",
            post(super::knowledge_api::register_model),
        )
        .route(
            "/api/v1/knowledge/register-bounty",
            post(super::knowledge_api::register_bounty),
        )
        .route(
            "/api/v1/knowledge/classes",
            get(super::knowledge_api::list_classes),
        )
        .route(
            "/api/v1/knowledge/predicates",
            get(super::knowledge_api::list_predicates),
        )
        // Reputation
        .route("/api/v1/reputation", get(super::reputation_api::show_reputation))
        .route(
            "/api/v1/reputation/attest",
            post(super::reputation_api::attest_agent),
        )
        .route(
            "/api/v1/reputation/leaderboard",
            get(super::reputation_api::leaderboard),
        )
        .route(
            "/api/v1/reputation/factors",
            get(super::reputation_api::show_factors),
        )
        // Model
        .route(
            "/api/v1/models",
        ...
        // Lineage
        .route(
            "/api/v1/lineage/register",
            post(super::lineage_api::register_lineage),
        )
        .route("/api/v1/lineage/{cid}", get(super::lineage_api::show_lineage))
        .route(
            "/api/v1/lineage/{cid}/parents",
            get(super::lineage_api::show_parents),
        )
        .route(
            "/api/v1/lineage/{cid}/children",
            get(super::lineage_api::show_children),
        )
        .route(
            "/api/v1/lineage/{cid}/ancestors",
            get(super::lineage_api::show_ancestors),
        )
        .route("/api/v1/lineage/{cid}/depth", get(super::lineage_api::show_depth))
        .route(
            "/api/v1/lineage/{cid}/royalties",
            post(super::lineage_api::compute_royalties),
        )
        ...
        .route(
            ...
            post(super::train_api::register_worker),
        )
        .route("/api/v1/train/workers", get(super::train_api::list_workers))
        .route(
            "/api/v1/train/coordinator-status",
            get(super::train_api::coordinator_status),
        )
        // Config
        .route("/api/v1/config", get(super::config_api::get_config).put(super::config_api::set_config))
        .route("/api/v1/config/path", get(super::config_api::config_path))
        // Lifecycle
        .route(
            "/api/v1/lifecycle/status",
            get(super::lifecycle_api::lifecycle_status),
        )
        .route(
            "/api/v1/lifecycle/activate",
            post(super::lifecycle_api::activate),
        )
        .route(
            "/api/v1/lifecycle/hibernate",
            post(super::lifecycle_api::hibernate),
        )
        .route(
            "/api/v1/lifecycle/reactivate",
            post(super::lifecycle_api::reactivate),
        )
