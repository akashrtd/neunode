...
- ⚠️ **The sovereign chain is experimental.** `agnetd serve --chain-mode sovereign` can drive Reth in single-validator mode or supervise a validator-specific pinned Malachite/Malaketh process. The off-chain Rust ledger remains the default canonical source of truth until the migration phases are complete; see [ADR-0001](./docs/adr/0001-canonical-ledger-source-of-truth.md).
